...
static void fot_sent_proc(u8 *buf,u8 len)
{
    ...
    if(fota_client_config){
        ...
        ble_notify_for_handle(connect_handle, ATT_CHARACTERISTIC_FF14_01_VALUE_HANDLE, buf, len);
    }
}
...
static void fot_reply_update_request(void)
{
    ...
    att_server_get_mtu(connect_handle, &packet_len);
    ...
    ble_update_conn_param(connect_handle, 12, 0, 400);
