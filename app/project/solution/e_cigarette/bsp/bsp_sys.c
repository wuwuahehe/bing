...
u8 heap_func[HEAP_FUNC_SIZE] AT(.func_heap);
u16 tmr5ms_cnt;
...
static void bsp_var_init(void)
{
    ...
    msg_queue_init();
    heap_malloc_init(heap_func, HEAP_FUNC_SIZE);
}
...
void thread_driver_event_callback(uint8_t msg)
{
    if (msg < THREAD_DRIVER_MSG_SMOKE_MAX) {
#if BSP_ECIGS_EN
        thread_driver_smoke_event_callback(msg);
#endif
    } else {
#if GUI_SELECT
        thread_driver_gui_event_callback(msg);
#endif
    }
...
#if OS_THREAD_DRIVER_STACK_EN
    thread_driver_user_callback_register(thread_driver_event_callback);
#endif
...
#if	DONGLE_AUTH_EN
    if (dongle_uid_entryption_check() == false) {
