...
#if (FLASH_SIZE > FSIZE_16M)

void spiflash_erase_for_4byte_addr(u32 addr);
bool spiflash_read_for_4byte_addr(void *buf, u32 addr, uint len, int encrypt_key, u16 crc_seed);
void spiflash_program_for_4byte_addr(void *buf, u32 addr, uint len);
void spiflash_erase_32k_for_4byte_addr( u32 addr);
void spiflash_erase_64k_for_4byte_addr( u32 addr);

AT(.com_text.util.spiflash)
bool spiflash_read(void *buf, u32 addr, uint len, int encrypt_key, u16 crc_seed)
{
    return spiflash_read_for_4byte_addr(buf, addr, len, encrypt_key, crc_seed);
}

AT(.com_text.spiflash.program)
void spiflash_program(void *buf, u32 addr, uint len)
{
    spiflash_program_for_4byte_addr(buf, addr, len);
}

AT(.com_text.spiflash.erase)
void spiflash_erase(u32 addr)
{
    spiflash_erase_for_4byte_addr(addr);
}

AT(.com_text.spiflash.erase_64k)
void spiflash_erase_64k(u32 addr)
{
    spiflash_erase_64k_for_4byte_addr(addr);
}

AT(.com_text.spiflash.erase_32k)
void spiflash_erase_32k( u32 addr)
{
    spiflash_erase_32k_for_4byte_addr(addr);
}


AT(.com_text.spiflash.addr.size)
bool spiflash_addr_4byte(void)
{
   return true;
}
