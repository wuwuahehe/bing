...
void deep_sleep_timer_cb(soft_timer_handle_t handle)
{
    // GPIO_PORT_GET(BSP_UART_DEBUG_EN)->de |= GPIO_PIN_GET(BSP_UART_DEBUG_EN);
    // printf("sleep timeout\n");
    sys_cb.sleep_prevent = 1;
    sys_cb.deep_sleep_flag = 1;
}


#if 0   /* Not support in Wireless Mode, please use bt_timer */
//休眠定时器，500ms进一次
AT(.com_sleep.sleep)
uint32_t deep_sleep_timer(void)
{
    uint32_t ret = 0;
...
static void sfunc_sleep(void)
{
    ...
    soft_timer_t deep_sleep_timer;
    soft_timer_create(&deep_sleep_timer, SYS_DEEP_SLEEP_TIME*1000, 1, deep_sleep_timer_cb);
    soft_timer_start(deep_sleep_timer.handle);
    ...
    soft_timer_destroy(&deep_sleep_timer);
