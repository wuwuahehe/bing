...
#if (AUDIO_ENC_TYPE == AUDIO_ENC_MSBC)
//MSBC
...
#elif (AUDIO_ENC_TYPE == AUDIO_ENC_ADPCM)
/*********************************************************************************************************
...
#endif //(AUDIO_ENC_TYPE == AUDIO_ENC_MSBC)
#else
...
uint16_t sdadc_output_buf[SDADC_OUTPUT_SIZE] AT(.wl.sdadc.obuf);
uint8_t enc_output_buf[ENC_OUTPUT_SIZE] AT(.wl.enc_obuf);

ring_buf_t sdadc_pcm_ring_buf AT(.wl.ring_buf);
uint8_t sdadc_pcm_buf[ENC_INPUT_SIZE * 2 * SDADC_BUF_CNT] AT(.wl.ring_buf);
...
void bsp_sdadc_voice_process(SDADC_DONE_TYPEDEF type)
{
    ...
    #if (AUDIO_ENC_TYPE == AUDIO_ENC_ADPCM)
