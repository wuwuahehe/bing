...
#define ATVV_ADV_DEBUG_EN 1

#if ATVV_ADV_DEBUG_EN
#define ATVV_ADV_DEBUG(...)                  printf(__VA_ARGS__)
#define ATVV_ADV_DEBUG_R(...)                print_r(__VA_ARGS__)
#else
#define ATVV_ADV_DEBUG(...)
#define ATVV_ADV_DEBUG_R(...)
#endif
...
uint8_t adv_data_poweron[] = {
    ...
    0x02, 0x0A, 0x00,
    //私有协议
    0x04, 0xFE, 0x6D, 0x4B, 0x5A,
};

//关机后广播
uint8_t adv_data_const_poweroff[] = {
    // Flags general discoverable, BR/EDR not supported
    0x02, 0x01, 0x04,
    // Manufacturer Specific Data
    0x03, 0xFF, 0x00, 0x00,
...
u32 ble_get_adv_data(u8 *adv_buf, u32 buf_size, LE_REMOTE_ADV_PKT adv_pkt)
{
    ...
    ATVV_ADV_DEBUG("%s, adv_pkt:%d, have_bd:%d\n", __func__, adv_pkt, have_bd);

    if ((adv_pkt == LE_REMOTE_ADV_PKT_DEFAULT) && !have_bd) {
        //上电无配对记录，自动开配对广播
        //上电有配对记录，开了定向广播则自动发定向广播（回连）；未开定向广播则不广播任何数据但是scan rsp包会返回蓝牙名，也会自动回连.
