...
#define LE_DEVICE_MAX_NUM             4  //最大绑定设备数，默认开8个设备的CM，设备超出8个需手动增加CM PAGE数
#define LE_DEVICE_MAX_BLOCK_LEN       (sizeof(le_device_memory_db_t) * (PAGE_DATA_SIZE/sizeof(le_device_memory_db_t)))  //每块page最大容纳设备信息量

le_device_memory_db_t le_devices[LE_DEVICE_MAX_NUM] AT(.ble_buf.att);
...
{
    if ((addr + size) <= (1 * LE_DEVICE_MAX_BLOCK_LEN)) {
        cm_read(buf, BLE_CM_PAGE(addr - (0 * LE_DEVICE_MAX_BLOCK_LEN)), size);
    } else if ((addr + size) <= (2 * LE_DEVICE_MAX_BLOCK_LEN)) {
        cm_read(buf, BLE_CM_1_PAGE(addr - (1 * LE_DEVICE_MAX_BLOCK_LEN)), size);
    }
}
...
{
    if ((addr + size) <= (1 * LE_DEVICE_MAX_BLOCK_LEN)) {
        cm_write(buf, BLE_CM_PAGE(addr - (0 * LE_DEVICE_MAX_BLOCK_LEN)), size);
    } else if ((addr + size) <= (2 * LE_DEVICE_MAX_BLOCK_LEN)) {
        cm_write(buf, BLE_CM_1_PAGE(addr - (1 * LE_DEVICE_MAX_BLOCK_LEN)), size);
    }
    cm_sync();
}
