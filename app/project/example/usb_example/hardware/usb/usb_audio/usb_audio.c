...
#define RING_BUF_LEN           0x960

//uda:usb device audio
...
epcb_t ep_isoc_in AT(.usb_buf.usb);
u8 ep_isoc_buf_in[AUDIO_ISOIN_SIZE + 2];
...
epcb_t ep_isoc_out AT(.usb_buf.usb);
u8 ep_isoc_buf_out[AUDIO_ISOOUT_SIZE + 4];
...
void usb_audio_in_callback(void);
void usb_audio_run_loop_execute(void);
...
AT(.text.func.usbdev)
void usb_audio_mic_start_do(void)
{
    /* Start mic */
...
AT(.text.func.usbdev)
void usb_audio_mic_stop_do(void)
{
    /* Sotp mic */
...
AT(.usbdev.com)
static void usb_audio_mic_stop(uda_t *uda)
{
    printf("%s\n", __func__);

    if (uda->flag_isocin_en) {
        usb_audio_mic_stop_do();
...
AT(.usbdev.com)
static void usb_audio_mic_start(uda_t *uda)
{
    printf("%s\n", __func__);

    if (uda->flag_isocin_en == 0) {
...
AT(.text.func.usbdev)
void usb_audio_speaker_start_do(void)
{
    /* Start mic */
...
AT(.text.func.usbdev)
void usb_audio_speaker_stop_do(void)
{
...
AT(.usbdev.com)
static void usb_audio_speaker_stop(uda_t *uda)
{
    printf("%s\n", __func__);

    usb_audio_speaker_stop_do();

    uda->flag_isocout_en = 0;
}

AT(.usbdev.com)
static void usb_audio_speaker_start(uda_t *uda)
{
    printf("%s\n", __func__);
    ...
    usb_audio_speaker_start_do();
}

///USB AUDIO iso out process(speaker)
AT(.usbdev.com)
void usb_audio_out_callback(void)
{
    uda_t *uda = &uda_0;
    u8 *aubuf = ude_aubuf;
    ...
    if (uda->spk_mute) {
        ...
        memcpy(aubuf, ep_isoc_buf_out, AUDIO_ISOOUT_SIZE);
    }
...
AT(.usbdev.com)
void usb_audio_speaker_sync(void)
{
...
void usb_audio_run_loop_execute(void)
{
    ...
    if (uda->flag_isocin_en != uda->flag_isocin_int) {
        if (uda->flag_isocin_int) {
            ...
            usb_audio_mic_start(uda);
        } else {
            usb_audio_mic_stop(uda);
        }
    }
#endif // USB_MIC_EN
    ...
    if (uda->flag_isocout_en != uda->flag_isocout_int) {
        if (uda->flag_isocout_int) {
            usb_audio_speaker_start(uda);
        } else {
            usb_audio_speaker_stop(uda);
        }
    }
#endif // USB_MIC_EN
}

void usb_audio_exit(uda_t *uda)
{
#if USB_MIC_EN
    if (uda->flag_isocin_en) {
        usb_audio_mic_stop(uda);
...
#if USB_MIC_EN
    uda_0.isoc_in  = &ep_isoc_in;
    memset(uda->isoc_in, 0x00, sizeof(epcb_t));
    usb_ep_callback_register(usb_audio_in_callback, USB_AUDIO_EP_IN_INDEX, EP_DIR_IN);
     //usb audio mic ep init
    memset(ep_isoc_buf_in,  0, sizeof(ep_isoc_buf_in));
    epcb = uda->isoc_in;
    epcb->dir = EP_DIR_IN;
    epcb->index = USB_AUDIO_EP_IN_INDEX;
    epcb->type = EP_TYPE_ISOC;
    epcb->epsize = AUDIO_ISOIN_SIZE;
    epcb->buf = ep_isoc_buf_in;
    epcb->first_pkt = 0;
    epcb->xlen = 0;
    usb_ep_init(epcb);
...
#if USB_SPEAKER_EN
    uda_0.isoc_out = &ep_isoc_out;
    memset(uda->isoc_out, 0x00, sizeof(epcb_t));
    usb_ep_callback_register(usb_audio_out_callback, USB_AUDIO_EP_OUT_INDEX, EP_DIR_OUT);
    //usb audio speaker ep init
    memset(ep_isoc_buf_out, 0, sizeof(ep_isoc_buf_out));
    epcb = uda->isoc_out;
    epcb->dir = EP_DIR_OUT;
    epcb->index = USB_AUDIO_EP_OUT_INDEX;
    epcb->type = EP_TYPE_ISOC;
    epcb->epsize = AUDIO_ISOOUT_SIZE;
    epcb->buf = ep_isoc_buf_out;
    usb_ep_init(epcb);
