...
const uint8_t usb_cfg_desc[] = {
    ...
    USB_DEV_STA,                /* Configuration characteristics: BusPowerd */
...
AT(.rodata.usb.desc)
const u8 usb_micr_os_str_desc[18] = {
    0x12,               // bLength
    ...
    STR_MS_VENDOR_CODE, // bMS_VendorCode
    0x00,               // bPad
};

AT(.rodata.usb.desc)
const u8 usb_micr_os_feature_desc[16] = {
    0x10, 0x00,         // -
    0x00, 0x00,         // dwLength
...
uint8_t *usb_get_microsoft_str_descriptor(uint8_t *length)
{
    *length = 18;
    memcpy(usb_str_desc_buf, usb_micr_os_str_desc, *length);
    return usb_str_desc_buf;
}

uint8_t *usb_get_microsoft_feature_descriptor(uint8_t *length)
{
    *length = 16;
    memcpy(usb_str_desc_buf, usb_micr_os_feature_desc, *length);
    return usb_str_desc_buf;
}
