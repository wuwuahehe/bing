...
void soft_timer_pwroff_callback(soft_timer_handle_t handle)
{
    GPIO_PORT_GET(BSP_UART_DEBUG_EN)->de |= GPIO_PIN_GET(BSP_UART_DEBUG_EN);
    printf("pwroff timeout\n");
    func_pwroff();
}
...
static void sfunc_sleep(void)
{
    ...
    #if SYS_OFF_TIME
    soft_timer_t soft_timer_pwroff;
    soft_timer_create(&soft_timer_pwroff, sys_cb.pwroff_delay * 100/* Unit: 100ms */, 0, soft_timer_pwroff_callback);
    soft_timer_start(soft_timer_pwroff.handle);
    #endif
...
#if SYS_OFF_TIME
    soft_timer_destroy(&soft_timer_pwroff);
#endif
