...
static list_t usb_usr_hid_queue_mouse;

static usb_hid_rep_mouse_t usb_usr_hid_pkts_pool_mouse[USB_USR_HID_CACHE_MOUSE];
...
static void usb_usr_hid_init(void)
{
    ...
    list_pool_init(
        &usb_usr_hid_queue_mouse,
        usb_usr_hid_pkts_pool_mouse,
        sizeof(usb_usr_hid_pkts_pool_mouse[0]),
        sizeof(usb_usr_hid_pkts_pool_mouse) / sizeof(usb_usr_hid_pkts_pool_mouse[0])
    );
}
...
static bool usb_usr_hid_queue_push(void *data)
{
    ...
    usb_hid_rep_mouse_t *free = (usb_hid_rep_mouse_t *)list_pop_front(&usb_usr_hid_queue_mouse);
    bool status = false;

    if (free != NULL) {
        memcpy(&free->mouse, data, sizeof(hid_rep_mouse_t));
...
static uint32_t usb_usr_hid_queue_pop(void *data)
{
    ...
    usb_hid_rep_mouse_t *elt = (usb_hid_rep_mouse_t *)list_pop_front(&usb_usr_hid_queue_send);
    uint32_t result = 0;

    if (elt != NULL) {
        if (data != NULL) {
            memcpy(data, &elt->mouse, sizeof(hid_rep_mouse_t));
        }
        list_push_back(&usb_usr_hid_queue_mouse, (list_hdr_t *)elt);
        result = sizeof(hid_rep_mouse_t);
    }
...
AT(.com_text.usb.hid.data.mouse)
uint8_t usb_usr_hid_mouse_write(hid_rep_mouse_t *mouse)
{
    ...
    if (!usb_usr_hid_queue_push(mouse)) {
