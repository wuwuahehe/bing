...
u32 ble_get_scan_data(u8 *scan_buf, u32 buf_size)
{
    ...
    if(at_cmd->scan_rsp_status==ENABLE || at_cmd->scan_rsp_len!=0)
    {
        if((data_len + at_cmd->scan_rsp_len) > 31)
        {
            printf("[err] scan response buffer insufficient!!!\n");
            return data_len;
        }
        memcpy(&scan_buf[data_len], at_cmd->scan_rsp_buf, at_cmd->scan_rsp_len);
        data_len += at_cmd->scan_rsp_len;
    }
